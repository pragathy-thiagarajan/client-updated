    ...
    <div className="relative flex min-h-[calc(100vh-65px)] items-center justify-center overflow-hidden bg-slate-50 px-4 py-12">
      ...
      <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white shadow-2xl shadow-slate-200/70 lg:grid-cols-[.9fr_1.1fr]">
        <div className="hidden bg-gradient-to-br from-slate-950 via-violet-950 to-violet-700 p-10 text-white lg:flex lg:flex-col lg:justify-between">
          <div>
            <span className="inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold ring-1 ring-white/20">EventHub</span>
            <h2 className="mt-8 text-4xl font-black tracking-tight">Your events. Your tickets. One place.</h2>
            <p className="mt-4 max-w-sm leading-7 text-violet-100/80">Sign in to manage bookings, access tickets and stay updated with every event you care about.</p>
          </div>
          <p className="text-sm text-violet-200/70">Secure access for attendees, organizers and admins.</p>
        </div>

        <form onSubmit={handleSubmit} className="p-7 sm:p-10 lg:p-12">
          <Link to="/" className="text-sm font-semibold text-violet-600 hover:text-violet-700">← Back home</Link>
          <h1 className="mt-8 text-3xl font-black tracking-tight text-slate-950">Welcome back</h1>
          ...
          <button type="submit" disabled={loading} className="primary-btn mt-7 w-full">
